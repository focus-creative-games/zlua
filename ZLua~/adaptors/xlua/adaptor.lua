-- ZLua xLua adaptor: CS.Namespace.Type → CSharp[assembly][typeFullName]
-- Spec: docs/spec/12-MIGRATION-ADAPTORS.md
-- No-namespace types: eager rawset on CS at init.
...
local pending = {}
-- [ns_table] = { [leaf_name] = { asm = string, full = string } } after lazy resolve
...
local function resolve_type(assembly_name, type_full_name)
    ...
    if asm == nil then
        error(string.format(
            "zlua xlua adaptor: assembly not found: %s (type %s)",
            tostring(assembly_name), tostring(type_full_name)))
    end
    local t = asm[type_full_name]
    if t == nil then
        error(string.format(
            "zlua xlua adaptor: type not found: CSharp[%q][%q]",
...
local function ensure_ns_table(parent, name)
    ...
    if cur == nil then
        -- May be a pending leaf on parent; promoting to namespace is a conflict.
        local parent_pending = pending[parent]
        if parent_pending and parent_pending[name] ~= nil then
            error(string.format(
                "zlua xlua adaptor: path conflict at %s (type leaf vs namespace)",
                tostring(name)))
        end
        cur = new_ns_table()
    ...
    if type(cur) ~= "table" then
        error(string.format(
            "zlua xlua adaptor: path conflict at %s (expected namespace table)",
            tostring(name)))
    end
    if pending[cur] == nil then
        -- Existing plain table (e.g. user-created): adopt as namespace.
        pending[cur] = {}
...
local function register_pending(ns_table, leaf, assembly_name, type_full_name)
    ...
    if existing ~= nil then
        if pending[existing] ~= nil then
            error(string.format(
                "zlua xlua adaptor: path conflict at %s (namespace vs type %s/%s)",
        ...
        if prev ~= nil and not same_owner(prev, assembly_name, type_full_name) then
            error(string.format(
                "zlua xlua adaptor: path conflict at %s: %s/%s vs %s/%s",
...
local function mount_type(root, assembly_name, type_full_name)
    ...
    if #parts == 0 then
        error(string.format(
            "zlua xlua adaptor: empty type name from %s / %s",
            assembly_name, tostring(type_full_name)))
    end

    if #parts == 1 then
        -- No namespace: eager register on root.
        local leaf = parts[1]
        ...
        if existing ~= nil and existing ~= type_table then
            error(string.format(
                "zlua xlua adaptor: path conflict at CS.%s (assembly %s, type %s)",
...
function M.init(export_types)
    ...
    for assembly_name, type_list in pairs(export_types) do
        if type(type_list) ~= "table" then
            error(string.format(
                "zlua xlua adaptor: export_types[%q] must be an array",
        ...
        for i = 1, #type_list do
            ...
            if type(full) ~= "string" or full == "" then
                error(string.format(
                    "zlua xlua adaptor: invalid type name in %s[%d]",
                    tostring(assembly_name), i))
            end
            mount_type(CS, assembly_name, full)
        end
    end
end
