...
static int ZLuaMakeGenericType(lua_State* L)
{
    ZLUA_TRY_BEGIN()
    if (!lua_istable(L, 1))
        LuaException::Throw("zlua.make_generic_type expects generic type table as first arg");
...
static int ZLuaToBytes(lua_State* L)
{
    ...
    if (obj->klass != MetadataUtil::GetByteArrayClass())
    {
        LuaException::ThrowFormat("zlua argument mismatch: expected byte array, got: %s.%s", obj->klass->namespaze, obj->klass->name);
    }

    Il2CppArray* array = (Il2CppArray*)obj;
    const char* data = (const char*)il2cpp_array_addr_with_size(array, 0, 1);
    lua_pushlstring(L, data, (size_t)array->max_length);
    return 1;
    ZLUA_TRY_END()
}
