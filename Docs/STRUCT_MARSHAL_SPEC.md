...
#### 6.2.3 Handle 路径（Il2Cpp）

C#→Lua 默认 **StructHandle**（lightuserdata）在同步调用链内可经 `StructValueOps::Resolve` 写回栈上 struct。

Lua→C# 的 `ref struct` 形参：

- 若传入 **StructUserData**：绑定 payload 指针（与 §5 路径 B 一致）。
- 若传入 **StructHandle**（由 C# 刚 Push、仍在 `StructStackScope` 内）：`Validate` + 取 `stack[index]` 地址作为 ref 目标（仅同步链）。
- 若传入 number / table 等：拷贝语义（§6.2.1），不使用 Handle。

Handle 与 StructUserData 互转规则不变：`ToUserData()` 为 **拷贝**，与 ref 槽独立。
