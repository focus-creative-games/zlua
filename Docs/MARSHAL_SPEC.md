# NovaLua Struct Marshal 设计规范

本文档描述 **Il2Cpp 与 Mono（Editor）** 下 **C# struct 与 Lua 互操作** 的传递方案，与 `DESIGN_SPEC.md`、`IL2CPP_DESIGN_SPEC.md` 中的 `Marshaling`、`LuaMarshalAs`、`ObjectRegistry` 体系衔接。
...
flowchart TB
    ...
    subgraph Native["novalua C++"]
        SSS["StructStackScope\nstack&lt;void*&gt; + generation"]
        SVO["StructValueOps\n(handle 操作 / ToUserData)"]
...
| 模块 | 职责 |
|------|------|
| `StructStackScope` | 记录栈上 struct 地址、分配 stackIndex、管理 generation、嵌套 scope |
| `StructValueOps` | 对 handleId 做字段 / 方法访问；`ToUserData()` 升级为 userdata |
...
```text
int64_t handleId = [ 32 bit generation | 32 bit stackIndex ]
```

- **stackIndex**：`stack` 向量下标，**不是**堆地址。
- **generation**：scope 批量失效时递增；过期 handle 校验失败。

Lua 侧：`lua_pushlightuserdata(L, (void*)(intptr_t)handleId)`。

### 3.3 Push（C# → Lua，Handle 路径）

前置条件：**struct 实例已在当前 native 栈帧**（IC 参数、局部变量、`ref` 指向的稳定区域）。

```text
PushStructHandle(Il2CppClass* klass, void* structPtr):
  1. 确保当前处于活跃 StructStackScope（见 3.5）
  2. stackIndex = stack.size()
  3. stack.push_back(structPtr)
  4. klassStack.push_back(klass)   // 若使用平行数组
  5. handleId = Encode(generation, stackIndex)
  6. lua_pushlightuserdata(L, handleId)
```

**零拷贝**：不把 struct 拷到池；blittable / non-blittable 均适用（non-blittable 的 GC 见 3.7）。
...
```text
ValidateHandle(handleId):
  - 解码 generation, stackIndex
  - generation == scope.generation  （否则 luaL_error: struct handle expired）
...
| `StructValueOps::ToUserData` 若使原 handle 失效 | 可选局部 | 迁移完成时 invalidate 单 handle |
...
- 长生命周期须用 **`StructUserData`** 或 **`ToUserData()`**。
...
## 4. StructValueOps（值类型操作类）

Lua / 绑定层通过 handleId 操作 struct，**不**直接解引用 lightuserdata 为地址。

```cpp
class StructValueOps {
public:
    static bool Validate(int64_t handleId);
    static void* Resolve(int64_t handleId, Il2CppClass* expectedKlass);

    // 字段 / 方法：Resolve 后 base + 预计算 offset，同 MetaBinding
    static int Index(lua_State* L, int64_t handleId, const char* key);
    static int NewIndex(lua_State* L, int64_t handleId, const char* key);

    // 升级为 userdata（见 4.1）
    static void ToUserData(lua_State* L, int64_t handleId);
};
```

metatable：`__index` / `__newindex` 从 lightuserdata 取出 handleId，转调 `StructValueOps`。

### 4.1 ToUserData() 语义（已采纳）

- **拷贝**：将 handle 解析到的 struct **复制** 一份到 userdata（blittable）或 `NotBlittableStructRegistry`（non-blittable）。
- 拷贝后 **handle 与 userdata 相互独立**；修改其一不影响另一，直至各自生命周期结束。
- 原 handle 仍在 `StructStackScope` 规则下有效（未 EndScope 前可继续使用）。
...
在 **`LuaAppDomain::Initialize` / `NovaLua` 启动时** 调用一次：
...
目标：**novalua 只调用公开 API**；Il2Cpp 内部把「回调注册表」与「Boehm 钩子」分离，使 **novalua 代码不直接改 `BoehmGC.cpp` 的业务逻辑**。

Il2Cpp 现有事实：`RegisterRoot` / `UnregisterRoot` 的**实现**在 `gc/BoehmGC.cpp` 的 `push_other_roots` 中，不在 `GarbageCollector.cpp`。因此「完全不碰 Boehm 后端」不可行；可行的是 **只改一处 Il2Cpp 基础设施，NovaLua 零侵入 Boehm**。

**推荐分层：**

```text
GarbageCollector.h          ← 公开 API（novalua 只 include 这个）
...
|--|-----------------------------------|------------------|
| novalua 改动 | 仅 novalua | novalua + Il2Cpp 基础设施一次 |
...
| 形态 | 说明 |
|------|------|
| **lightuserdata** | 仅当由 C#→Lua Handle 传入；Pop 时 `Validate` + `memcpy` |
| **StructUserData** | userdata 拷贝；Pop 时读 payload |
...
| C# 参数 | 行为 |
|---------|------|
| by-value | Pop 时拷贝进栈上临时 / 参数槽 |
| `ref` / `out` | Lua 侧修改经 `Resolve` 写回；Pop 结束时同步到 C# `ref` 目标 |
...
- **Handle**：lightuserdata + metatable → `StructValueOps`。
- **UserData**：payload + 与 class 相同的 `instanceMap`（offset 注册期预计算）。
- **禁止** 通过实例访问 static 成员。
- 方法 `this`：Handle → `Resolve(handleId)`；UserData → `payload` 地址。
...
[LuaInvoke IC]
  ...
  PushStructHandle(&localStruct) → h
  lua_pcall(...)                        // Lua 可用 h，StructValueOps 校验
  guard.~()                             // EndScope: 截断 stack, ++generation

Lua 保存 h 至下次调用 → Validate 失败 → error

需长期持有 → ToUserData() 或 [LuaMarshalAs(StructUserData)]
```
...
| ToUserData 与 handle 双写 | **拷贝**语义，两者独立 |
...
|------|--------|------|------------|
| **StructHandle** | `StructStackScope` + lightuserdata(handleId) | 等价语义实现 | `lightuserdata`，opaque handle |
| **StructUserData** | `NotBlittableStructRegistry` + userdata | 等价语义实现 | `userdata` + struct metatable |
| **ToUserData()** | 拷贝到 userdata | 拷贝到 userdata | handle → userdata，两者独立 |
| **StructValueOps** | 字段 / 方法 / 校验 | 同 API 与 metatable | 相同 `__index` 行为 |
...
- handleId 编码、过期报错文案、`ToUserData` 拷贝语义与 Il2Cpp **不一致**。

### 13.3 类型注册（与 class 并列）

- `CSharp.*` 懒注册 value type：`__valuetype = true`。
- 实例 metatable：**Handle** 与 **UserData** 各一套，与 `MetaBinding` 字段 / 方法表同名。
...
4. 默认 table：`foo({ x=, y= })`  
5. `ToUserData()` 后修改 userdata 不影响原 handle（拷贝）  
...
| 阶段 | Il2Cpp | Mono |
|------|--------|------|
| Phase 1 | `StructStackScope` + Handle + `StructValueOps` | 同等 Lua 语义的最小实现 |
...
2. **handleId**：opaque lightuserdata，不提供 Lua 侧构造。
3. **ToUserData**：**拷贝**到 userdata；与 handle 独立共存。
