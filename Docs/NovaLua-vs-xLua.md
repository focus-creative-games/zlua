...
本文档从 **架构、调用路径、理论开销** 三方面对比 NovaLua 与 xLua（Tencent，Unity Il2Cpp 常用方案）。用于技术选型与设计评审。

**说明：**

- 开销数据为 **路径级推演 + 经验量级**，非仓库内 benchmark。
- NovaLua 对比对象为其 **Player / Il2Cpp 设计目标**；当前实现仍为 MVP（见 `IL2CPP_DESIGN_SPEC.md`）。
- 性能对比 **仅对 Player Il2Cpp 有意义**；NovaLua Editor（Mono 反射）通常 **不比 xLua 快**。
...
NovaLua 与 xLua 同属 **高性能原生绑定** 路线，而非 HybridCLR 式执行模型改造。
...
NovaLua MVP 示例（`MethodBridge.cpp`）对 `Add(int,int)` 即为：`lua_tointeger` ×2 → `methodPointer` → `lua_pushinteger`，无 Wrap、无 libxlua 往返。
...
绑定完成后，两者都要走「C# 调 invoke → 进 Lua `pcall`」；NovaLua 少 C# Wrap / LuaDLL 层，但 **string / 重对象** 仍占主导。
...
- **总耗时倍数** = T_xLua / T_NovaLua（越大 NovaLua 越快）
...
| NovaLua | `__index` → C++ offset getter |
| 总耗时 | **约 5–15×** |
| 互调开销削减 | **约 85%–95%** |
...
| 场景 | 总耗时倍数 (xLua/NovaLua) | 互调开销大约减少 |
|------|---------------------------|------------------|
...
NovaLua **刻意避免**运行时伪造完整 `MethodInfo`（见 `FUNCTION_MARSHAL_SPEC.md`），在技巧深度上有所克制。
...
NovaLua 热路径 **接近** 该下限；xLua 因多次 LuaDLL，通常 **高一个数量级**。

---

## 10. 建议基准测试（待实测）

在 **相同 Il2Cpp Player** 下对比 xLua 与 NovaLua：
