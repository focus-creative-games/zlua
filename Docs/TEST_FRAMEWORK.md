...
| API | 说明 |
|-----|------|
| `EnsureInitialized()` | 幂等；确保 `LuaAppDomain` 已初始化、加载 `bootstrap.lua`（依赖 NovaLua 内置 `require`） |
...
`LuaAppDomain.Initialize(moduleLoader)` 时，`LuaEnv.SetModuleLoader` 会向 `package.searchers` 注册 NovaLua 模块 searcher（位置 2），并通过 C# 回调 `__novalua_load_module` 调用 `moduleLoader`：
