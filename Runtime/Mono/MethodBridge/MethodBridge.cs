...
{
    internal static class MethodBridgeCore
    {
        ...
        /// <summary>
        /// 在创建类�?Lua 表之前校验别名键；失败时�?<see cref="InvalidOperationException"/>�?        /// 避免部分注册�?C# 异常污染 Lua 栈�?        /// </summary>
        ...
        /// <summary>
        /// 将已通过 <see cref="ValidateMethodAliasKeys"/> 的别名写入元表�?        /// 别名与非别名方法名的冲突检查在该校验中完成，此处不再重复�?        /// </summary>
        ...
        /// <summary>
        /// 注册期校验：别名键与默认方法名（非别名）必须互不相交；别名之间亦须唯一�?        /// </summary>
        ...
        internal static void PushArgumentValue(IntPtr luaState, object value, Type type)
        {
            ...
            {
                StructMarshaling.PushUnderlyingInteger(luaState, Enum.GetUnderlyingType(targetType), value);
                return;
            }
            ...
            if (StructMarshaling.IsStructType(targetType))
            {
            ...
            {
                TypeRegistry.PushConstructorInstance(luaState, value, ResolveInstancePushType(targetType, value));
                return;
            }
        ...
        private static void PushBoxedObjectValue(IntPtr luaState, object value)
        {
            ...
            {
                StructMarshaling.PushUnderlyingInteger(luaState, Enum.GetUnderlyingType(runtimeType), value);
                return;
            }
        ...
        private static int RegisterMethod(MethodInfo method, bool isStatic)
        {
            ...
            if (isStatic)
            {
                ...
                if (MethodBridgeFactory.TryCreate(method, isStatic: true, out LuaCSFunction fastInvoker))
                {
            ...
            else
            {
                ...
                if (MethodBridgeFactory.TryCreate(method, isStatic: false, out LuaCSFunction fastInvoker))
                {
        ...
        private static int RegisterConstructor(ConstructorInfo ctor)
        {
            ...
            if (ConstructorBridge.TryCreate(ctor, argStartIndex: 2, out LuaCSFunction callInvoker))
            {
        ...
        private static int RegisterDispatchGroup(DispatchGroup group)
        {
            ...
            {
                if (ConstructorOverloadDispatch.TryCreate(
                        group,
                        dispatchId,
                        argStartIndex: 2,
            ...
            out MethodInfo method)
        {
            ...
            for (int i = 0; i < methods.Count; i++)
            {
                ...
                if (!string.Equals(
                        TypeRegistry.FormatParameterSignature(candidate.GetParameters()),
                        parameterSignature,
        ...
        private static void PushDirectCFunction(IntPtr luaState, LuaCSFunction invoker)
        {
            ...
            IntPtr fn = global::System.Runtime.InteropServices.Marshal.GetFunctionPointerForDelegate(invoker);
        ...
        private static int InvokeConstructorInstance(IntPtr luaState, Type type, ConstructorInfo ctor, object[] args)
        {
            ...
            return TypeRegistry.PushConstructorInstance(luaState, instance, type);
        }
        ...
        private static int InvokePreparedConstructorCall(IntPtr luaState, ConstructorInfo ctor, object[] args)
        {
            try
            {
                StructOpaqueScope.EnterLuaToCSharp();
                object instance = ctor.Invoke(args);
                return TypeRegistry.PushConstructorInstance(luaState, instance, ctor.DeclaringType);
            }
            finally
            {
                StructOpaqueScope.LeaveLuaToCSharp();
            ...
            Type effectiveClrType = null)
        {
            ...
            {
                return ValueMarshaling.CanConvert(luaState, luaIndex, parameterType, marshalType);
            }
            ...
            Type effectiveClrType = null)
        {
            ...
            if (marshalType != LuaMarshalType.Default)
            {
                if (!ValueMarshaling.CanConvert(luaState, luaIndex, parameterType, marshalType))
                {
                    return false;
                }

                value = ValueMarshaling.Read(luaState, luaIndex, parameterType, marshalType);
                return true;
            }
        ...
        private static bool CanConvertLuaValue(IntPtr luaState, int luaIndex, Type parameterType)
        {
            ...
            if (targetType.IsEnum)
            {
                ...
                {
                    return StructMarshaling.TryReadUnderlyingInteger(luaState, luaIndex, Enum.GetUnderlyingType(targetType), out _);
                }
            ...
            if (StructMarshaling.IsStructType(targetType))
            {
                ...
                {
                    return StructMarshaling.TryGetBoxedTarget(luaState, luaIndex, out object boxed)
                        && boxed != null
        ...
        private static object ReadValue(IntPtr luaState, int luaIndex, Type type)
        {
            ...
            if (targetType.IsEnum)
            {
                if (StructMarshaling.TryReadEnumValue(luaState, luaIndex, targetType, out object enumValue))
                {
                    return enumValue;
                }
            ...
            if (targetType.IsArray)
            {
                if (StructMarshaling.TryGetBoxedTarget(luaState, luaIndex, out object arrayValue)
            ...
            if (targetType.IsClass || targetType.IsInterface)
            {
                if (StructMarshaling.TryGetBoxedTarget(luaState, luaIndex, out object referenceValue)
                    && referenceValue != null
        ...
        private static object ReadDelegateValue(IntPtr luaState, int luaIndex, Type delegateType)
        {
            ...
            if (luaType == LuaDataType.UserData)
            {
                if (StructMarshaling.TryGetBoxedTarget(luaState, luaIndex, out object boxed)
                    && boxed is Delegate del
        ...
        private static int PushReturn(IntPtr luaState, MethodInfo method, object ret)
        {
            ...
            if (marshalType != LuaMarshalType.Default)
            {
                return ValueMarshaling.PushReturn(luaState, returnType, ret, marshalType);
            }
        ...
        private static int PushReturnDefault(IntPtr luaState, Type returnType, object ret)
        {
            ...
            if (unwrappedReturnType.IsEnum)
            {
                ...
                StructMarshaling.PushUnderlyingInteger(luaState, Enum.GetUnderlyingType(unwrappedReturnType), enumValue);
                return 1;
            }
            ...
            if (StructMarshaling.IsStructType(unwrappedReturnType))
            {
                if (IsNullNullableReturn(returnType, ret))
                {
                    LuaDll.lua_pushnil(luaState);
                ...
                return TypeRegistry.PushConstructorInstance(luaState, ret, unwrappedReturnType);
            }
            ...
            {
                return TypeRegistry.PushConstructorInstance(luaState, ret, ResolveInstancePushType(unwrappedReturnType, ret));
            }
