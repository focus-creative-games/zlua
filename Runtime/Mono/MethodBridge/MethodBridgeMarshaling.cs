...
namespace ZLua.MethodBridge
{
    ...
    /// </summary>
    internal static class MethodBridgeMarshaling
    {
        private static void EnsureArgumentType(IntPtr luaState, int index, Type parameterType)
        {
            if (!MethodBridgeCore.CanConvertArgumentValue(luaState, index, parameterType))
            {
        ...
        internal static void ValidateExactArgCount(IntPtr luaState, int argStartIndex, int expectedCount, MethodInfo method)
        {
            ...
            if (luaArgCount != expectedCount)
            {
                LuaDllExtension.error(
                    luaState,
                    $"zlua: argument mismatch for {method.DeclaringType.FullName}.{method.Name}{MethodBridgeCore.GetParameterSignaturePublic(method.GetParameters())}");
            }
        }
        ...
        internal static void ValidateExactArgCount(IntPtr luaState, int argStartIndex, int expectedCount, ConstructorInfo ctor)
        {
            ...
            if (luaArgCount != expectedCount)
            {
                LuaDllExtension.error(
                    luaState,
                    $"zlua: argument mismatch for {ctor.DeclaringType.FullName}..ctor{MethodBridgeCore.GetParameterSignaturePublic(ctor.GetParameters())}");
            }
        }
        ...
        internal static object PopTarget(IntPtr luaState, int index, Type targetType)
        {
            ...
            return MethodBridgeCore.ReadValuePublic(luaState, index, targetType);
        }

        internal static int PushReturn(IntPtr luaState, Type returnType, object value)
        {
            return MethodBridgeCore.PushReturnDefaultPublic(luaState, returnType, value);
        }
        ...
        {
            if (!StructMarshaling.TryGetBoxedTarget(luaState, targetIndex, out object target))
            {
            ...
            try
            {
                value = MethodBridgeCore.ReadValuePublic(luaState, valueIndex, field.FieldType);
            }
            catch (Exception ex)
            {
        ...
        internal static int SetBoxedInstanceProperty(IntPtr luaState, PropertyInfo property, int targetIndex, int valueIndex)
        {
            ...
            try
            {
                value = MethodBridgeCore.ReadValuePublic(luaState, valueIndex, property.PropertyType);
            }
            catch (Exception ex)
            {
                return LuaCallbackBoundary.ToLuaError(luaState, ex);
