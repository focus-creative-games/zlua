...
namespace ZLua.MethodBridge
{
    ...
    /// </summary>
    internal static class ConstructorBridge
    {
        ...
        private static readonly MethodInfo PushConstructorInstanceMethod = typeof(TypeRegistry).GetMethod(
            nameof(TypeRegistry.PushConstructorInstance),
            BindingFlags.NonPublic | BindingFlags.Static,
            binder: null,
        ...
        internal static bool CanFastInvoke(ConstructorInfo ctor)
        {
            ...
            for (int i = 0; i < parameters.Length; i++)
            {
                ...
                if (!MethodBridgeFactory.CanFastType(parameter.ParameterType))
                {
                    return false;
                }
            }

            return MethodBridgeFactory.CanFastType(ctor.DeclaringType);
        }
