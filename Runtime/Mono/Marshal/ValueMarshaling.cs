...
{
    internal static class ValueMarshaling
    {
        ...
        {
            if (!StructMarshaling.IsStructType(structType))
            {
        ...
        private static bool CanConvertUserData(IntPtr luaState, int luaIndex, Type targetType, LuaDataType luaType)
        {
            ...
            {
                return StructMarshaling.TryGetBoxedTarget(luaState, luaIndex, out _);
            }

            return StructMarshaling.TryReadBoxedValueUserData(luaState, luaIndex, targetType, out _);
        }
        ...
        private static object ReadUserData(IntPtr luaState, int luaIndex, Type targetType)
        {
            ...
            if (StructMarshaling.TryReadBoxedValueUserData(luaState, luaIndex, targetType, out object primitive))
            {
                return primitive;
            }
        ...
        private static void PushUserData(IntPtr luaState, object value, Type targetType)
        {
            ...
            if (targetType.IsEnum)
            {
                ...
                TypeRegistry.PushConstructorInstance(luaState, value, targetType);
                return;
            }
            ...
            if (targetType == typeof(object))
            {
                ...
                if (actualType.IsEnum || StructMarshaling.IsStructType(actualType)
                    || actualType.IsArray || typeof(Delegate).IsAssignableFrom(actualType)
                    || actualType.IsClass || actualType.IsInterface)
                {
                    TypeRegistry.PushConstructorInstance(luaState, value, ResolveInstancePushType(actualType, value));
                    return;
                }

                StructMarshaling.PushBoxedValueUserData(luaState, value);
                return;
            }
